        ...
        for scale = scale_all
            for id_blur = 1:num_blur
                ...
                for idx_im = 1:length(filepaths)
                    ...
                    im_SR = imread(fullfile('./SR', [methods{idx_method}], dataset{idx_set}, ['x', num2str(scale)], blur{id_blur}, name_SR));
                    ...
                    % fprintf(results, 'x%d bi_blur:%s %d %s: PSNR= %f SSIM= %f\n', scale, blur{id_blur}, idx_im, name_SR, PSNR_all(idx_im), SSIM_all(idx_im));
                    % fprintf('x%d bi_blur:%s %d %s: PSNR= %f SSIM= %f\n', scale, blur{id_blur}, idx_im, name_SR, PSNR_all(idx_im), SSIM_all(idx_im));
                end
                fprintf(results, '--------Mean--------\n');
                fprintf('--------Mean--------\n');
                fprintf(results, 'x%d bi_blur:%s: PSNR= %f SSIM= %f\n', scale, blur{id_blur}, mean(PSNR_all), mean(SSIM_all));
                fprintf('x%d bi_blur:%s: PSNR= %f SSIM= %f\n', scale, blur{id_blur}, mean(PSNR_all), mean(SSIM_all));
            end
        end
    end
end
