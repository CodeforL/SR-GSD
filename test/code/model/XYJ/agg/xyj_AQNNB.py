...
class QNNA(nn.Module): #QNNS
    def __init__(
        self, conv, n_feat,
        bias=True, bn=False, act=nn.ReLU(True)):
        
        super(QNNA,self).__init__()
		...
		self.max3_1 = nn.MaxPool2d(3,stride=1,padding=1)
...
class Bone(nn.Module):
    def __init__(
        self, conv, n_feat,isQNNA=False,aggregation=False,
        bias=True, bn=False, act=nn.ReLU(True)):
        ...
        if QNNA:
            self.block1 = QNNA(conv, n_feat, bias, bn, act)
            self.block2 = QNNA(conv, n_feat, bias, bn, act)
        else:
            self.block1 = QNNB(conv, n_feat, bias, bn, act)
            self.block2 = QNNB(conv, n_feat, bias, bn, act)
...
class HDA(nn.Module):
    def __init__(
        ...
        for i in range(layer-1):
            if i == 0:
                modules_body.append(Bone(conv, n_feat,False,False,bias, bn, act))
            else:
                modules_body.append(Bone(conv, n_feat,False,True ,bias, bn, act))
        self.body = nn.Sequential(*modules_body)
        self.qnnb1 = QNNB(conv, n_feat, bias, bn, act)
        self.qnnb2 = QNNB(conv, n_feat, bias, bn, act)
