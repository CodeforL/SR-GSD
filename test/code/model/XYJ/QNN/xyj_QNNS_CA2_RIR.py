...
class QNNS(nn.Module): #QNNS
    ...
    def forward(self,x):
        ...
        index2 = self.ca2(self.conv2_0(temp1) + temp2)
        ...
        res = x + self.conv(torch.cat([index1,index2,index3,index4,index5,index6],1))
...
class ResidualGroup(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, act, n_resblocks):
        ...
        modules_body = [
            QNNS(
                conv, n_feat, bias=True, bn=False, act=nn.ReLU(True)) \
            for _ in range(n_resblocks)]
